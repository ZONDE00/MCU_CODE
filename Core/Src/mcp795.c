...
int mcp795_read_time(mcp795_t *dev) {
	...
	dev->rtc_time->milliseconds = ((received[2] & 0xf0) >> 4) * 10
			+ (received[2] & 0x0f);
	dev->rtc_time->seconds = ((received[3] & 0x70) >> 4) * 10
			+ (received[3] & 0x0f);
	dev->rtc_time->minutes = ((received[4] & 0x70) >> 4) * 10
			+ (received[4] & 0x0f);
	dev->rtc_time->hours = (((received[5] & 0x30) >> 4) * 10)
			+ (received[5] & 0x0f);
	dev->rtc_time->days = received[6] & 0x07;
	dev->rtc_time->date = (((received[7] & 0x30) >> 4) * 10)
			+ (received[7] & 0x0f);
	dev->rtc_time->month = (((received[8] & 0x10) >> 4) * 10)
			+ (received[8] & 0x0f);
	dev->rtc_time->year = ((received[9] & 0xf0) >> 4) * 10
			+ (received[9] & 0x0f);
	return status;
}
